...
const MoreToys = () => {
  ...
  const filteredToys = toys.filter((toy) => toy.toyId >= 0);
  ...
  return (
    <div className="bg-gray-100 mx-auto min-h-screen py-8 px-24">
      <h1 className="text-3xl font-bold text-center mb-8 text-blue-700">
        All Toys
      </h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-4 justify-items-center">
        {filteredToys.map((toy) => (
          <div
            key={toy.toyId}
            className="bg-white shadow-md rounded-2xl p-4 text-center transition-transform transform hover:scale-105 w-90"
          >
            <img
              src={toy.pictureURL}
              alt={toy.toyName}
            ...
            <div className="flex justify-between">
              ...
              <div className="flex justify-center m-2">
                  <button onClick={() => handleAddToCart(toy)} className="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-lg transition">
                    Add to Cart
                  </button>
              </div>
            </div>
